...
function TermCard({ term, explanation }: MedicalTerm) {
  return (
    <motion.div
      ...
      exit={{ opacity: 0, x: 10 }}
      className="bg-card rounded-xl border border-border p-3.5 shadow-sm hover:shadow-md transition-shadow"
    >
...
export function MedicalTermsPanel({ selectedBlock, globalTerms, globalKeywords, onClearSelection }: MedicalTermsPanelProps) {
  ...
  return (
    <div role="complementary" aria-label={t('terms.panel.aria')} className="flex flex-col gap-4 h-full">
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0">
          <h2 className="text-sm font-bold text-foreground">
            {isBlockSelected ? t('terms.selected') : t('terms.title')}
          </h2>
        ...
        <AnimatePresence mode="wait">
          {terms.length > 0 ? (
            ...
            >
              ...
              <p className="text-xs">{t('terms.empty')}</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      {keywords.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-muted-foreground mb-2">{t('terms.keywords')}</p>
