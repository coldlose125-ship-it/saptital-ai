...
export type SpeechStatus = 'idle' | 'recording' | 'processing' | 'error' | 'unsupported';
...
export function useSpeechRecognition(
  ...
  useEffect(() => {
    ...
    if (!SpeechRecognition) {
      ...
      setErrorMsg('speech.unsupported');
    ...
    recognition.onerror = (event: any) => {
      ...
      if (event.error === 'not-allowed') {
        setErrorMsg('speech.mic.denied');
      } else if (event.error === 'network') {
        setErrorMsg('speech.network');
      } else {
  ...
  const startListening = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setErrorMsg('speech.mic.denied2');
        return;
      }
    }
